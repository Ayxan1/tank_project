...
class Tank():
    ...
    def check_free_angle_for_moving_forward(self):
        ...
        if self.bottom_wall_collision == True:
            if (abs(angle) < 90 and abs(angle) > 0) or (abs(angle) > 270 and abs(angle) < 360):
                print('*************************************')
                print('bottom collision  anglee ---- ', angle)
                print('*************************************')
                self.bottom_wall_collision = False
    ...
    def update(self):
        if self.moving_right:
            ...
            print('tank center x coordinate  -  ', self.rect.centerx+20)
            print('tank center y coordinate  -  ', self.rect.centery+20)
        if self.moving_left:
            self.angle += 2
            # new_image = pygame.transform.rotate(image_clean, self.angle)
            image = pygame.transform.rotozoom(self.image, self.angle, 1)
        ...
        if self.moving_forward:
            ...
            if self.angle == 0 or self.angle == 360 or self.angle == -360 or self.angle == 180 or self.angle == -180 :
                ...
                print('tank center x coordinate  -  ', self.rect.centerx+20)
                print('tank center y coordinate  -  ', self.rect.centery+20)
                print('screen bottom  -  ', self.screen_rect.bottom)
                print('screen right  -  ', self.screen_rect.right)
                print('screen angle  -  ', self.angle)
                print('________________________________________________________________')
                print('collisions right_wall_collision ', self.right_wall_collision)
                print('collisions left_wall_collision ', self.left_wall_collision)
                print('collisions top_wall_collision ', self.top_wall_collision)
                print('collisions bottom_wall_collision ', self.bottom_wall_collision)
